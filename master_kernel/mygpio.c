...
#define ECHO 29 //this is gpio input, ECHO
#define TRIG 30 //this is gpio output, TRG
...
static void trig1_timer_handler(unsigned long data) {
	...
	pxa_gpio_set_value(TRIG, 1);
	jiffy1=jiffies;
	//printk( "trig1 while before\n");
	printk("");
	while (jiffies-jiffy1<msecs_to_jiffies(0.01));
	//printk( "trig1 while after\n");
	printk("");
	pxa_gpio_set_value(TRIG, 0);
...
static int my_init_module(void)
{
	...
	/* Registering device */
	  ...
	  proc_entry = create_proc_entry( "sensor", 0644, NULL );
	  if (proc_entry == NULL) {
	    printk(KERN_INFO "sensor: Couldn't create proc entry\n");
...
static void my_cleanup_module(void)
{
	...
	remove_proc_entry("sensor", &proc_root);
